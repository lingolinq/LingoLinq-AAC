    ...
    vocabularies.each do |vocab|
      ...
      begin
        ...
        if filename.end_with?('.obz')
          ...
          # Run full post_process on root board to build ButtonSet and downstream links
          root_board = result.first
          if root_board
  ...
  task :import_single, [:vocab_name] => :environment do |t, args|
    ...
    begin
      ...
      if filename.end_with?('.obz')
        ...
        result.settings['unlisted'] = false
        result.generate_stats
        # Save with full post_process to build ButtonSet
        result.instance_variable_set(:@buttons_changed, 'import')
        result.instance_variable_set(:@brand_new, true)
        result.save!
        puts "  Imported 1 board (with button set)"
      end
