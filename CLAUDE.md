...
- Console access: use `bin/audit_console` (sets `USER_KEY` so console record-writes are attributed to you via PaperTrail, and works from the Render Shell tab, a Cloud Run exec shell, or locally). The audited-session control is **operative** as of 2026-08-29 (LL-7f7372e3eb, verified-closed) and is not wrapper-only: `bin/rails` runs `Audit::ConsoleGuard.enforce_pre_boot!` before the app boots, and `bundle exec rails console` re-execs through `bin/rails` (`Rails::AppLoader.exec_app`), so both paths are covered. In production an un-keyed `console`/`runner` is refused pre-boot, `db`/`dbconsole` is refused outright (HIPAA), and `config/initializers/auditing.rb` re-checks at runtime (catching `-e`/`--environment` forms) before writing a session-open `AuditEvent` fail-closed. The wrapper's job is to prompt for `USER_KEY` rather than let you hit that refusal. **SCOPE, i.e. what it does NOT cover:** the hooks are `Rails.application.console` and `.runner`, so they fire for those two commands only. `bin/rake` requires `config/boot` directly and invokes no guard, and a direct boot (`ruby -r./config/environment -e '...'`) bypasses `bin/rails` entirely; both give full database access with no `AuditEvent`, no refusal and no attribution, in production included. Read "audited console" as covering `rails console` and `rails runner`, not app access generally. RESIDUAL: `USER_KEY` is self-asserted free text, not derived from an authenticated principal, so the attributed actor is spoofable by anyone who already has a shell on the app; recorded in LL-7f7372e3eb's closure evidence
