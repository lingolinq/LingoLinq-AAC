...
> console`, so it works from a Cloud Run exec shell or a local checkout.
> `USER_KEY` does two things: it attributes record writes to you via PaperTrail, and it satisfies the audited-session control, which records a
