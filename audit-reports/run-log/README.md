...
- Logs the tool name and the repo-relative **path/glob** a finder Read/Grep'd/Glob'd, plus a
  **redacted, truncated** Bash command.
...
- Bash commands are redacted for secret/PII shapes (API-key prefixes, JWT-ish blobs, long
  hex/base64, emails, `KEY=value`, `--password X`) before logging - the "shape not values"
  discipline (finding LL-b5c30235d3).
