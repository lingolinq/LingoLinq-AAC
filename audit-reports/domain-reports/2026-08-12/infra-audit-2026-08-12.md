...
- **Residual:** Residual, not yet addressed: dump.rdb (blob 4c38846c751ebed1ef1c8095aaa9682af47783d3) remains fully retrievable from public git history at commit ced829ba1 (2026-01-30), which is an ancestor of both origin/staging and origin/main in a PUBLIC GitHub repo (verified 2026-08-16 via `git merge-base --is-ancestor` and `gh repo view --json visibility`). git rm --cached only stops future commits from tracking it; it does not remove it from history. 'remediated-unverified' describes the forward-fix (ignored going forward) and the content inspection (confirmed clean), not the historical exposure, which stays open pending a history-rewrite decision.
