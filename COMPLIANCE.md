...
| **Google Cloud Platform** (Cloud Run / Cloud SQL / Memorystore / Vertex AI) | Production compute + managed Postgres/Redis once the Render-to-GCP cutover lands (**not yet carrying prod data; PLANNED, not active**) | **BAA ACCEPTED (console, 2026-07-12)** | Google Cloud HIPAA BAA + Cloud Data Processing Addendum accepted 2026-07-12, SCCs certified 2026-07-14, on project `lingolinq-prod` (formalizes the org-wide Google BAA of 2026-06-08). Console acceptance, no countersigned PDF; recorded in `docs/legal/GCP_BAA_ACCEPTED.md`. PHI permitted under BAA terms, which are necessary but not sufficient: HIPAA-eligible services only, encryption in transit and at rest, access controls, minimum necessary; private VPC additionally. Google becomes an **active** subprocessor in `docs/legal/SUBPROCESSORS.md` only at cutover. Infrastructure BAA only; does NOT cover the Anthropic AI-egress path (Vertex AI is in scope but not a live inference path, disabled 2026-07-09 PR #570). |
| **Anthropic (Claude)** | **Runtime:** pseudonymized (scrubbed) prompts for AI board generation + word prediction (Haiku 4.5) and eval-narrative drafting (Opus 4.7), redacted by `lib/pii_scrubber.rb` before egress. Also dev-time code assistance. | No model-provider BAA (open item) | Runtime AI egress is governed by the PiiScrubber + no-identifiable-data policy, not a BAA; scrubbed learner content remains pseudonymized personal data. HIPAA posture on this path is provisional pending CEO review (see `docs/legal/AI_GOVERNANCE_MEMO.md`, `SUBPROCESSORS.md` row 4). ZDR confirmed for Haiku 4.5 + Opus 4.7 only. |
| **Google (Gemini)**  | **Runtime fallback DISABLED 2026-07-09** (PR #570); no active runtime data flow today. Dev-time code assistance only. | N/A (no active runtime flow) | If reactivated (e.g. Vertex AI under the GCP infrastructure BAA), re-review data-handling terms before any child data; tracked as `rev-gemini-baa-annual`. See `SUBPROCESSORS.md` row 5. |
...
| A3 | **ALWAYS** execute BAAs with Render, AWS, and GCP before handling HIPAA data. | AWS BAA signed 2026-02-07 (see `docs/legal/`). GCP HIPAA BAA + CDPA accepted in-console 2026-07-12, SCCs 2026-07-14, on project `lingolinq-prod` (Cloud Run migration target; PHI permitted under BAA terms, necessary but not sufficient; see `docs/legal/GCP_BAA_ACCEPTED.md`). Render BAA deferred as a pre-MVP gate; must be executed before any paying institutional contract. |
