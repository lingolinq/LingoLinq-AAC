...
| **Google Cloud Platform** (Cloud Run / Cloud SQL / Memorystore) | Production compute + managed Postgres/Redis once the Render-to-GCP cutover lands (not yet carrying prod data) | **BAA SIGNED (2026-07-12)** | Google Cloud HIPAA BAA + Cloud Data Processing Addendum accepted 2026-07-12, SCCs certified 2026-07-14, on project `lingolinq-prod` (formalizes the org-wide Google BAA of 2026-06-08). PHI permitted on GCP under BAA terms (HIPAA-eligible services, encryption, private VPC). Google becomes an active subprocessor in `docs/legal/SUBPROCESSORS.md` at cutover. Evidence: Drive "Compliance Audits" / "Google Cloud Platform - Accepted Compliance Agreements (captured 2026-07-14)". Infrastructure BAA only; does NOT cover the Anthropic AI-egress path. |
...
| A3 | **ALWAYS** execute BAAs with Render, AWS, and GCP before handling HIPAA data. | AWS BAA signed 2026-02-07 (see `docs/legal/`). GCP HIPAA BAA + CDPA signed 2026-07-12, SCCs 2026-07-14, on project `lingolinq-prod` (Cloud Run migration target; PHI permitted under BAA terms). Render BAA deferred as a pre-MVP gate; must be executed before any paying institutional contract. |
