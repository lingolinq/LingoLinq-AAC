...
var contentGrabbers = Service.extend({
      ...
      object.save().then(function (object) {
        ...
        if (object.get('pending')) {
          ...
          if (!object.get('data_url') && original_url) {
            get_data_url = persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(original_url), {
              type: 'GET'
            }).then(function (data) {
              return data.data;
            });
  ...
  file_selected: function (type, files) {
    ...
    if (type == 'image' || type == 'avatar' || type == 'badge') {
      ...
      if (window.cg.log_controller) {
        window.cg.log_controller.send('import', files[0]);
      }
    } else {
...
var pictureGrabber = EmberObject.extend({
  ...
  file_selected: function (file, type) {
    ...
    if (file && file.localURL) {
      ...
      reader = window.cg.read_file(file);
    }
    var size = null;
    if (type == 'avatar' && window.cg.avatar_result) {
      window.cg.avatar_result(true, 'loading');
      if (window.cg.avatar_result.size) {
        size = window.cg.avatar_result.size;
      }
    } else if (type == 'badge' && window.cg.badge_result) {
      window.cg.badge_result(true, 'loading');
    }
    ...
    var force_data_url = sizer.then(function (res) {
      if (res.url && res.url.match(/^data/)) {
        ...
        return window.cg.read_file(file).then(function (data) {
    ...
    force_data_url.then(function (res) {
      ...
      if (type == 'avatar' || type == 'badge') {
        ...
        window.cg.save_record(image).then(function (res) {
          if (type == 'avatar' && window.cg.avatar_result) {
            window.cg.avatar_result(true, res);
          } else if (type == 'badge' && window.cg.badge_result) {
            window.cg.badge_result(true, res);
          } else {
            console.error("nothing to handle successful image upload");
          }
        }, function (err) {
          if (type == 'avatar' && window.cg.avatar_result) {
            window.cg.avatar_result(false, err);
          } else if (type == 'badge' && window.cg.badge_result) {
            window.cg.badge_result(false, err);
          } else {
  ...
  find_picture: function (text, user_name, locale) {
    if (text && (text.match(/^http/))) {
      ...
      persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(text), {
        type: 'GET'
      }).then(function (data) {
        _this.controller.set('image_preview', {
          url: data.data,
      ...
      var _this = this;
      if (!persistenceService.get('online')) {
  ...
  picture_search: function (library, text, user_name, locale, fallback, safe, skin) {
    ...
    return search(text, user_name, locale, safe).then(function (data) {
      skin = skin || 'default';
      var which_skin = null;
      if ((skin && skin != 'default') || appStateService.get('currentUser.preferences.skin')) {
        var s = (skin && skin != 'default') ? skin : appStateService.get('currentUser.preferences.skin');
      ...
      data.forEach(function (img) {
        ...
        if (img.skins) {
          ...
          } else if (appStateService.get('currentUser.preferences.skin')) {
  ...
  protected_search: function (text, library, user_name, locale, fallback) {
    ...
    var _this = this;
    return persistenceService.ajax('/api/v1/search/protected_symbols?library=' + encodeURIComponent(library) + '&q=' + encodeURIComponent(text) + '&user_name=' + encodeURIComponent(user_name), {
      type: 'GET'
    }).then(function (data) {
      data.forEach(function (img) {
  ...
  open_symbols_search: function (text, user_name, locale, safe) {
    ...
    return persistenceService.ajax(path, {
      type: 'GET'
    }).then(function (data) {
      return data;
    }, function (xhr, message) {
  ...
  flickr_search: function (text) {
    ...
    return persistenceService.ajax('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + window.flickr_key + '&text=' + text + '&safe_search=1&media=photos&license=2%2C3%2C4%2C5%2C6%2C7&extras=license%2C+owner_name&format=json&nojsoncallback=1', {
      type: 'GET'
    }).then(function (data) {
      var res = [];
  ...
  openclipart_search: function (text) {
    return persistenceService.ajax('https://openclipart.org/search/json/?query=' + text + '&amount=30', {
      type: 'GET'
    }).then(function (data) {
      var res = [];
  ...
  pixabay_search: function (text, filter) {
    ...
    return persistenceService.ajax('https://pixabay.com/api/?key=' + window.pixabay_key + '&q=' + text + '&image_type=' + type + '&per_page=30&safesearch=true', {
      type: 'GET'
    }).then(function (data) {
      var res = [];
  ...
  public_domain_image_search: function (text) {
    ...
    return persistenceService.ajax('https://www.googleapis.com/customsearch/v1?q=' + text + '&cx=014949904870326116447%3Awlqrabmc6cu&rights=cc_publicdomain&safe=medium&key=' + window.custom_search_key, {
      type: 'GET'
    }).then(function (data) {
      var res = [];
      console.log(data);
  ...
  edit_image_preview: function () {
    ...
    (new RSVP.Promise(function (resolve, reject) {
      if (preview.url.match(/^http/)) {
        persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(preview.url), {
          type: 'GET'
        }).then(function (data) {
          resolve(data.data);
        }, function (xhr, message) {
  ...
  start_webcam: function () {
    ...
    capabilities.permissions.assert('record_video').then(function (res) {
      if (navigator.device && window.cg.capture_types().image) {
        ...
        var last_stream_id = stashesService.get('last_stream_id');
...
var videoGrabber = EmberObject.extend({
  ...
  file_selected: function (file) {
    var _this = this;
    var reader = window.cg.read_file(file);
    reader.then(function (data) {
      window.cg.read_file(file, 'blob').then(null, function () { return RSVP.resolve(null); }).then(function (blob) {
        var res = {
  ...
  recorder_available: function () {
    return !!(capabilities.subsystem != 'Kindle' && (navigator.getUserMedia || window.cg.capture_types().video));
  },
  record_video: function () {
    ...
    capabilities.permissions.assert('record_video').then(function (res) {
      if (window.cg.capture_types().video) {
  ...
  user_media_ready: function (stream, stream_id) {
    ...
    mediaRecorder.stopped = function () {
      ...
      videoGrabber.recorded_blobs = [];
      var reader = window.cg.read_file(blob);
      reader.then(function (data) {
  ...
  select_video_preview: function () {
    ...
    _this.controller.send('video_pending');
    ...
    var save_video = video_load.then(function (data) {
      ...
      return window.cg.save_record(video);
    });

    save_video.then(function (video) {
      _this.controller.set('video', video);
      _this.clear_video_work();
      _this.controller.send('video_ready', video.get('id'));
    }, function (err) {
      err = err || {};
      err.error = err.error || "unexpected error";
      lingoLinqExtras.track_error("upload failed: " + err.error);
      ...
      _this.controller.send('video_not_ready');
    });
  },
  save_pending: function () {
    var _this = this;
...
var soundGrabber = EmberObject.extend({
  ...
  recording_selected: function (file) {
    ...
    if (type == 'sound') {
      ...
      // do the hard stuff
      var progress = window.cg.upload_for_processing(file, '/api/v1/sounds/imports', {}, progressor);
  ...
  recorder_available: function () {
    return !!(capabilities.subsystem != 'Kindle' && (navigator.getUserMedia || window.cg.capture_types().audio));
  },
  record_sound: function (auto_loading) {
    ...
    capabilities.permissions.assert('record_audio').then(function (res) {
      ...
      } else if (!auto_loading && window.cg.capture_types().audio) {
  ...
  native_record_sound: function () {
    var _this = this;
    if (window.cg.capture_types().audio) {
  ...
  toggle_recording_sound: function (action) {
    ...
    if (action != 'stop' && !mr && !navigator.getUserMedia && window.cg.capture_types().audio) {
...
var boardGrabber = EmberObject.extend({
            ...
            progress_tracker.track(data.meta.progress, function (event) {
              if (event.status == 'errored') {
                ...
                event.result.board.forEach(function (board) {
                  result.push(LingoLinq.store.push({
                    data: {
                      id: board.id,
                      type: 'board',
                      attributes: board
  ...
  build_board: function () {
    ...
    if (appStateService.get('currentUser.known_supervisees')) {
      var sups = appStateService.get('currentUser.known_supervisees');
      if (sups.length > 0) {
  ...
  copy_found_board: function () {
    ...
    var editable_supervisee_author = (appStateService.get('currentUser.known_supervisees') || []).find(function (s) { return s.edit_permission && s.user_name == source_board_user_name; });
    var new_author = (editable_supervisee_author || {}).user_name || appStateService.get('currentUser.user_name');
  ...
  pick_board: function (board, force) {
    ...
    var current_user_name = appStateService.get('currentUser.user_name');
    var editable_supervisee_author = (appStateService.get('currentUser.known_supervisees') || []).find(function (s) { return s.edit_permission && s.user_name == source_board_user_name; });
...
var linkGrabber = EmberObject.extend({
  ...
  find_apps: function () {
    ...
    if (os == 'ios' || os == 'android') {
      var lookup = persistenceService.ajax('/api/v1/search/apps?q=' + encodeURIComponent(q) + '&os=' + os, {
        type: 'GET'
      });
...
document.addEventListener('change', function (event) {
  ...
  if (uploadIds.indexOf(event.target.id) !== -1) {
    ...
    window.cg.file_selected(type, files);
  }
});
...
document.addEventListener('paste', function (event) {
  if (document.getElementById('find_picture')) {
    ...
    if (found_file) {
      window.cg.file_pasted('image', data);
      event.preventDefault();
...
document.addEventListener('drop', function (event) {
  ...
  if (buttonElem) {
    ...
    window.cg.content_dropped(id, event.dataTransfer);
  } else if (document.getElementById('find_picture')) {
    if (pictureGrabber.controller.get('model.id')) {
      window.cg.content_dropped(pictureGrabber.controller.get('model.id'), event.dataTransfer);
    }
...
capabilities.data_uri_to_blob = function (uri) {
  if (window.cg && window.cg.data_uri_to_blob) {
    return window.cg.data_uri_to_blob(uri);
  }
  return null;
};

contentGrabbers.reopen({
  boardGrabber: boardGrabber,
