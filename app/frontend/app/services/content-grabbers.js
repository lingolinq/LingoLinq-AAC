      ...
      object.save().then(function(object) {
        ...
        if(object.get('pending')) {
          ...
          if(!object.get('data_url') && original_url) {
            get_data_url = persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(original_url), { type: 'GET'
            }).then(function(data) {
              return data.data;
            });
          }
...
var pictureGrabber = EmberObject.extend({
  ...
  find_picture: function(text, user_name, locale) {
    if(text && (text.match(/^http/))) {
      ...
      persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(text), { type: 'GET'
      }).then(function(data) {
        _this.controller.set('image_preview', {
          url: data.data,
...
//      search = function(str) { return _this.open_symbols_search(str, 'pcs'); }
  ...
  open_symbols_search: function(text, user_name, locale, safe) {
    ...
    return persistenceService.ajax(path, { type: 'GET'
    }).then(function(data) {
      return data;
    }, function(xhr, message) {
      var error = i18n.t('image_not_available', "Image retrieval failed unexpectedly.");
  ...
  flickr_search: function(text) {
    ...
    return persistenceService.ajax('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + window.flickr_key + '&text=' + text + '&safe_search=1&media=photos&license=2%2C3%2C4%2C5%2C6%2C7&extras=license%2C+owner_name&format=json&nojsoncallback=1', { type: 'GET'
    }).then(function(data) {
      var res = [];
  ...
  openclipart_search: function(text) {
    return persistenceService.ajax('https://openclipart.org/search/json/?query=' + text + '&amount=30', {type: 'GET'
    }).then(function(data) {
      var res = [];
  ...
  pixabay_search: function(text, filter) {
    ...
    return persistenceService.ajax('https://pixabay.com/api/?key=' + window.pixabay_key + '&q=' + text + '&image_type=' + type + '&per_page=30&safesearch=true', { type: 'GET'
    }).then(function(data) {
      var res = [];
  ...
  public_domain_image_search: function(text) {
    ...
    return persistenceService.ajax('https://www.googleapis.com/customsearch/v1?q=' + text + '&cx=014949904870326116447%3Awlqrabmc6cu&rights=cc_publicdomain&safe=medium&key=' + window.custom_search_key, { type: 'GET'
    }).then(function(data) {
      var res = [];
      console.log(data);
  ...
  edit_image_preview: function() {
    ...
    (new RSVP.Promise(function(resolve, reject) {
      if(preview.url.match(/^http/)) {
        persistenceService.ajax('/api/v1/search/proxy?url=' + encodeURIComponent(preview.url), { type: 'GET'
        }).then(function(data) {
          resolve(data.data);
        }, function(xhr, message) {
  ...
  save_image_preview: function(preview, force_content_type) {
    ...
    var button = editManager.find_button(_this.controller.get('model.id'));
