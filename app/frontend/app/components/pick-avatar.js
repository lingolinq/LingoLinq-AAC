...
/**
 * Choose profile picture modal. Rendered via modal service (modal-container);
 * outlet-based route.render path is not used when the modal service exists.
 */
export default Component.extend({
    ...
    function() {
      ...
      res.forEach(function(option) {
        ...
        emberSet(
          ...
          'div_style',
          htmlSafe(
            'height: 0; width: 100%; padding-bottom: 100%; overflow: hidden; background-position: center; background-repeat: no-repeat; background-size: contain; background-image: url(' +
              url +
              ');'
          )
        );
  ...
  actions: {
    ...
    save_edit() {
      ...
      editManager.get_edited_image().then(
        function(url) {
          ...
          contentGrabbers.save_record(image).then(
            function(res) {
              _this.set('saving', false);
