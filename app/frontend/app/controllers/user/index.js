  ...
  actions: {
    ...
    resendConfirmation: function () {
      this.persistence.ajax('/api/v1/users/' + this.get('model.user_name') + '/confirm_registration', {
        ...
        this.modal.success(i18n.t('confirmation_resent', "Confirmation email sent, please check your spam box if you can't find it!"));
      }, function () {
        ...
        this.persistence.ajax('/api/v1/users/' + this.get('model.user_name') + '/subscription', {
          ...
          progress_tracker.track(data.progress, function (event) {
            if (event.status == 'errored') {
              ...
              this.modal.success(i18n.t('subscription_updated', "User purchase information updated!"));
            }
          });
        }, function () {
          _this.set('subscription_settings.loading', false);
    ...
    rename_user: function (confirm) {
      if (confirm === undefined) {
        ...
        this.persistence.ajax('/api/v1/users/' + this.get('model.user_name') + '/rename', {
          ...
          runLater(function () {
            this.modal.success(i18n.t('user_renamed_to', "User successfully renamed to %{k}. The full renaming process can take a little while to complete.", { k: res.key }));
          }, 200);
        }, function (err) {
