...
> `docs/legal/COPPA_VERIFICATION_2026-04-26.md` (`DOC-407d2c2bf4`, ATTESTED 2026-07-23 by Scot
> Wahlquist, CEO; first attested 2026-06-21), which remains frozen and byte-identical. That
> predecessor's bytes still match its recorded `attestedContentHash`, so this is a content
> correction, not tampering.
>
...
> or that its citations still resolve. Current status for anything here lives in
> `audit-reports/FINDINGS.json`, which is the source of truth; this file is evidence of what was
> found, when.
>
> ## Reason for supersession (2026-09-17): the predecessor's banner asserted a scheduled job was
> running when it was not
>
> The predecessor's banner stated that `AiApiLog.redact_old_ip_addresses!` "was wired into
> `lib/tasks/scheduler.rake` by PR #222 and **runs daily today**". That sentence was introduced
...
> because the register's attestation note for that revision records the re-attestation as
> following "verification against live code and infrastructure state".
>
> **What is accurate, as of 2026-09-17:**
>
> - The wiring is real. `AiApiLog.redact_old_ip_addresses!` is called at
>   `lib/tasks/scheduler.rake:187`, inside the block gated on `hour == 6` UTC
>   (`lib/tasks/scheduler.rake:140`). The method is defined at `app/models/ai_api_log.rb:225`
>   with a 90-day default window.
> - It did **not** run between 2026-07-21 and 2026-09-02, because nothing invoked the dispatcher.
> - It has been **observed running daily since 2026-09-03**. Across the twelve UTC dates
>   2026-09-03 through 2026-09-14 it reported **0 AI log IPs redacted** on each date. A zero is the
>   count of rows updated; it does not establish that no qualifying rows ever existed, only that
>   none qualified at each observed execution. Evidence and its limits are recorded in
...
> The finding is recorded under **Phase 2B, "Item 3 - Retention Policy Enforcement"**, and appears
> again in the Phase 2B coverage summary, the Phase 3 punch list (row 3b) and the 2026-04-27
> re-verification section.
>
> **Scope of this supersession.** Only the banner is rewritten. The body below is reproduced from
> the predecessor and is unchanged: it is a point-in-time record of the 2026-04-27 verification and
> is not re-verified here. No finding is closed, downgraded or accepted, and
> `audit-reports/FINDINGS.json` remains authoritative.
>
> **Attestation history of the predecessor, retained for the record:** first attested 2026-06-21.
> PR #597 (2026-07-13) corrected one line (the COPPA parental-consent default is ON, not opt-in),
> so the attested revision no longer existed. Re-attested 2026-07-23 by Scot Wahlquist, CEO,
> against the then-current revision, with the point-in-time framing added. Those attestations
> belong to the predecessor's bytes, not to these.
>
> **ATTESTER MUST CONFIRM:** this record is a member of the `school-dpa-package` bundle and is
> Drive-mirrored, so the corrected text is exported to school districts. The predecessor's Drive
> mirror still carries the inaccurate sentence and is not updated by this commit; refreshing or
> retiring that mirror is a separate act.
