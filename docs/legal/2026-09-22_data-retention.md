...
> frozen and byte-identical. That predecessor was accurate about Render, which was already gone when it
> was written. This successor exists for a different reason: three of its rows described retention
> controls the code does not implement, as the rationale block below sets out. Every attestation statement reproduced below belongs to a predecessor version and is
...
>
> Every other row is carried forward from the predecessor, including its own
...
>
> **Rows changed beyond the three above, disclosed 2026-09-25.** An earlier draft of this block
> said every other row was carried forward *byte-identical*, which was not true of three cells and
> would have been frozen by attestation. The `LogSnapshot` mechanism cell was rewritten from
> "**None.** ... is not swept by `Flusher`" to the **Partial** wording now in section 2, because
> `Flusher.flush_user_content` has swept it since `eec90591a`; its retention-window cell was
> corrected at the same time, having been left contradicting the mechanism beside it. The two
> `AiApiLog` cells were updated where they described `LL-3e36a18199` as open, which it has not been
> since Scot closed it on 2026-09-18. No other cell differs from the predecessor.
>
...
PR #721. That draft was itself superseded, and this 2026-09-22 record is the one now awaiting CEO attestation.
...
|---|---|---|---|---|
| User account records (`users` table and dependents) | Active for the life of the account. After **12 months with no write to the account record** a deletion warning cycle begins, and the account is then permanently erased. Shorter paths exist: see the mechanism and notes cells | GDPR Article 5(1)(e); FERPA; contract | `User.check_for_subscription_updates` (`app/models/concerns/subscription.rb`) selects `updated_at < 12.months.ago`, sends **two** `deletion_warning` emails at least 3 weeks apart (attempts 1 and 2), then on the next eligible pass takes the `attempts > 2` branch, which sets `schedule_deletion_at = 36.hours.from_now` and sends `account_deleted`; the daily `flush_users` step of `scheduler:dispatch` runs `Flusher.flush_deleted_users`, which **enqueues** `Flusher.flush_user_completely` on a worker (500 rows per run) | **CORRECTED 2026-09-22.** Every predecessor read "Active for the life of the account plus 2 years of inactivity, then soft-delete", with `Flusher.flush_user_completely` named as the mechanism. Both halves of the window were wrong: the code window is 12 months, and the outcome is permanent destruction (`flush_record` plus an `AuditEvent` of type `user_permanently_destroyed`), not a soft delete that masks PII. Exempt from the sweep: accounts with `preferences['never_delete']`, accounts with `allow_log_reports` whose `updated_at` is within 36 months, and the `testing@example.com` fixture accounts. **This is not the only automatic destruction path, and 12 months is not the shortest.** An expired free trial (`expires_at < 1.week.ago` and `updated_at < 4.weeks.ago` with `billing_state == :expired_communicator`) enters the same warning cycle, reaching erasure in roughly 10 weeks. `schedule_deletion_at` is also written by self-serve account deletion (`app/controllers/api/users_controller.rb`) and by **COPPA offboarding consent** (`app/models/user.rb`, `app/workers/offboarding_coppa_expiration_worker.rb`), which its own comment records as irreversible once it fires. **Those two COPPA routes are not equally live, corrected 2026-09-25.** A parental-consent DECLINE schedules deletion immediately and is ungated. The daily EXPIRY sweep is not an operative destruction path by default: `OffboardingCoppaExpirationWorker.mode` returns `:disabled` unless `COPPA_OFFBOARDING_SWEEP_ENABLED` is set to `true` or `report`, and both `:disabled` and `:report` schedule nothing by construction. That variable is set nowhere in this repository, so on any deployment that has not set it explicitly the expiry route destroys no account. The attester must confirm the deployed value before this row is read as an operative window. Note also that the window measures `users.updated_at`, a row-write timestamp rather than a usage timestamp, and the warning loop itself rewrites it. Execution is subject to the same scheduled-dispatch caveat recorded in the `LogSession` row below |
| Communication logs (`LogSession`) | **No default window, and no general purge.** Runs only for users sponsored by an organization that sets `retention_months`, and only for 5 of the log types | GDPR Article 5(1)(e); FERPA; customer contract | `lib/data_policy_enforcer.rb`, dispatched daily at 06:00 UTC via `scheduler:dispatch` (configured cadence; production scheduled dispatch of `rake scheduler:dispatch` was interrupted from 2026-07-21 to 2026-09-02 (finding `LL-3e36a18199`, closed 2026-09-18; the impact of that interruption is still unassessed under open finding `LL-cbc8bc4211`), so this task was not run by the scheduler in that window; whether it ran by any other route has not been established. Captures dated 2026-09-14 record hourly execution from 2026-09-02 and one run of the daily 06:00 UTC block on each of the twelve UTC dates 2026-09-03 through 2026-09-14. Across September 3 to 14, each of the six inspected task summaries appeared once per UTC date and reported zero for its stated result. Six of the eleven daily tasks were inspected; the remainder were not queried. These observations do not verify every daily task, do not establish stored population size or contents, and do not evidence completion of downstream asynchronous work. See `docs/legal/2026-09-14_scheduler-dispatch-interruption-and-restoration.md`.) | **CORRECTED 2026-09-22.** Every predecessor read "3 years default, configurable per user or organization via data policy". Three things were wrong. (1) No default exists: `lib/data_policy_enforcer.rb` skips any organization whose policy sets no positive `retention_months` (`next unless months && months > 0`). (2) There is **no per-user path at all**: the job iterates `Organization.where("data_policy_version > 0")` and then that org's `sponsored_users`, so a standalone account is never reached (open finding `LL-d8072299bf`). (3) Only `RETAINABLE_LOG_TYPES` (`session`, `note`, `assessment`, `eval`, `journal`) are purged; `daily_use`, `modeling_activities` and `profile` sessions are deliberately excluded and have **no** automated purge, including the `profile` sessions whose identifiable summaries are cached into `UserExtra` (open findings `LL-1890f6a922`, `LL-de9c94bf36`, and `LL-caf2528468` for the profile cache). A configured window is enforced within that scope, and district customers commonly set 5 years |
| Communication snapshots (`LogSnapshot`) | **Erased with the account; no cascade from the parent log** | GDPR Article 17; FERPA | **Partial.** `LogSnapshot` is keyed by `user_id`, not by `LogSession`, and has no `dependent: :destroy` and no database foreign key. `Flusher.flush_user_content` does sweep it on full account erasure (`lib/flusher.rb`, since `eec90591a`, 2026-09-03), but nothing removes it when the parent log is hard-deleted | Open finding LL-1e2ab28aab. Individually deletable by the user via the snapshots API; a hard delete leaves the row (label, date range, `device_id`, dangling `location_id`) |
...
Known gaps tracked for remediation: the Flusher cascade must be verified against any newly added model. A 2026 April audit flagged `License`, `UserVideo`, `UserExtra`, `AiApiLog`, `ContactMessage`, and `LogSnapshot`; `License`, `UserVideo`/`ButtonSound`, and `AiApiLog` now have explicit Flusher handling. `LogSnapshot` is swept on full account erasure but not on a hard delete of the parent log (open finding LL-1e2ab28aab). `UserExtra` and `ContactMessage` remain unverified: no claim is made here either way. These gaps are tracked in the findings register (`audit-reports/FINDINGS.json`), which is the single source of truth for remediation status; there is deliberately no separate gap tracker, because a second list would drift from the register.

Retention enforcement jobs are **configured to run** under `lib/tasks/scheduler.rake` at 06:00 UTC daily. Production scheduled dispatch of `rake scheduler:dispatch` was interrupted from 2026-07-21 to 2026-09-02 (finding `LL-3e36a18199`, closed 2026-09-18; the impact of that interruption is still unassessed under open finding `LL-cbc8bc4211`), so this task was not run by the scheduler in that window; whether it ran by any other route has not been established. Captures dated 2026-09-14 record hourly execution from 2026-09-02 and one run of the daily 06:00 UTC block on each of the twelve UTC dates 2026-09-03 through 2026-09-14. Across September 3 to 14, each of the six inspected task summaries appeared once per UTC date and reported zero for its stated result. Six of the eleven daily tasks were inspected; the remainder were not queried. These observations do not verify every daily task, do not establish stored population size or contents, and do not evidence completion of downstream asynchronous work. See `docs/legal/2026-09-14_scheduler-dispatch-interruption-and-restoration.md`. The `DataPolicyEnforcer` reads the org-level effective data policy and applies that organization's window to its sponsored users. It does **not** reach a child organization that inherits a policy rather than setting `data_policy_version` itself, which is the hierarchy shape district customers use (open finding `LL-14edf1a801`).
