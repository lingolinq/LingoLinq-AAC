...
**Two distinct failure directions (do not conflate them).** Every read path (`public_view`, `marked?`, and the cloner via `normalized`) re-verifies the signature server-side, and a marker that does not verify is treated as **unmarked**, never as "verified AI-generated." So the **verification-failure** limitations (8.1 export, 8.2 key rotation) degrade toward **under-marking**: the system stops claiming AI provenance it cannot prove. Under-marking is the direction Art. 50(2) polices, so each is tracked with an explicit remediation trigger. **The bearer/transplant property (8.3) is the opposite direction and must not be glossed:** because the signature binds provenance, not content, a genuine, still-valid marker can be moved onto content it did not originate from, which is a **false-marking** vector (human-authored content reading as AI-generated). That is called out separately below per the explicit instruction in `lib/art50_marker.rb` that it "must be documented in the compliance record, not over-claimed as content integrity."
...
- **Risk assessment.** **Low real-world harm:** transplant requires write access to the target board, so the realistic case is a user marking their **own** human-authored board as AI-generated (self-inflicted over-disclosure), not an attacker forging a third party's provenance. The signature still cannot be minted or altered without the server secret, so the marker remains unforgeable; only its content binding is absent.
