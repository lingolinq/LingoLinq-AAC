...
| 5 | Misses the real entrypoint entirely: never touches `boards_controller#generate_labels` then `generate_words`. Nothing ever sets `article_50_disclosure_shown = true`. | `app/controllers/api/boards_controller.rb:569` (entrypoint; the Article 50 backstop it now carries is at `:592`) |
