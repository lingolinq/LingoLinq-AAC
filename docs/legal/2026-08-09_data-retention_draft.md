...
| AI API logs (`AiApiLog`), EU-jurisdiction accounts | Up to 5 years; purge job **wired, currently a no-op** | EU AI Act Article 50 record-keeping | `AiApiLog.purge_old_eu_logs!(years: 5)` (PR #553), daily via `scheduler:dispatch` | **CORRECTED 2026-08-25.** This cell previously read "Now functional ... It matches EU rows wherever Phase 4 is deployed", and column 2 read "**enforced**". Both overstate. The job is scheduled, and the three AI call sites do stamp `jurisdiction` via `LingoLinq::Article50CallContext` (`lib/ai_word_predictor.rb:423`, `lib/ai_board_generator.rb:698`, `lib/eval_narrator.rb:331`). But the job deletes `jurisdiction = 'EU' AND created_at < 5.years.ago` (`app/models/ai_api_log.rb:244-248`), and the `jurisdiction` column did not exist until 2026-06-21 (`db/migrate/20260621120000_add_article_50_fields_to_ai_api_logs.rb`). No stamped row can satisfy the age predicate before **2031-06-21**, so the purge matches zero rows today and every day until then. "Enforced" describes the schedule, not any deletion. |
