...
> **v1.3.2 DRAFT - awaiting attestation. Internal use only until the CEO attests this file.**
> Successor via Path A supersession to `docs/legal/2026-08-22_compliance-program.md`
> (v1.3.1, `DOC-e5e85eccb1`), which remains frozen and byte-identical and was accurate for its
> date: Render was still a live write-frozen rollback fallback on 2026-08-22. **This successor
> exists because that stopped being true on 2026-09-09**, when the Render workspace was deleted.
> The predecessor's snapshot boundary `64cdccba1` is retained for every count and finding figure;
> the Render decommission is a later infrastructure FACT recorded on top of that snapshot, not a
> re-derivation of it. See correction 5 below. That predecessor in turn succeeded attested
> `docs/legal/2026-08-20_compliance-program.md`
...
| 5 | Render described as a live write-frozen rollback fallback (:308-313, :334, :419, :424, :436, :453), and "Render BAA is pending" (:259) | **Render was decommissioned and the workspace deleted 2026-09-09** (0 services, 0 databases, 0 disks). The fallback no longer exists, so it is not a rollback option, not a subprocessor, and not a pending-BAA item. The gate row moves from Pending to Complete. This is a fact dated AFTER the `64cdccba1` snapshot; it changes no count, finding id or framework figure derived from that snapshot. The attester must confirm the deletion date against the vendor record. Findings LL-aacae48768 and LL-7314b5a8ea / LL-107c9fb665 become eligible for disposition, but are NOT closed here: only the CEO closes a finding, and `audit-reports/FINDINGS.json` remains authoritative. |
...
- **Subprocessors.** The Article 28 / 45 CFR 164.502(e) register is `docs/legal/SUBPROCESSORS.md`,
  ...
  correction. The Render BAA question is moot as of 2026-09-09: Render was decommissioned and is no
  longer a subprocessor, so there is no hosting-provider BAA gap on that path. GCP is the
  hosting provider under the accepted HIPAA BAA.
...
accepted-risk) and the audited-console finding (LL-7f7372e3eb, **verified-closed 2026-08-29**) were
held open pending exactly this event. The precondition is now satisfied, so LL-aacae48768 is
eligible for supersession, as are LL-7314b5a8ea and LL-107c9fb665 whose evidence anchor was
`render.yaml`. **None of them is closed by this document.** Only the CEO closes a finding and
...
> `ruby -r./config/environment` boot, and USER_KEY remains self-asserted.
