...
dormant, not un-inerted. **Added 2026-09-14:** wiring and configured cadence are distinct from execution; see the dispatch-interruption note at the "IP redaction leg" clarification below and `docs/legal/2026-09-14_scheduler-dispatch-interruption-and-restoration.md`. Two separate clocks, and an earlier version of this
correction conflated them: the STAMP begins writing `'EU'` on the first AI call logged for a confirmed `:eu` user (the stamp is written by the `AiApiLog` wrappers, so an EU user who makes no AI call produces no stamped row)
...
(`db/migrate/20260621120000`). So no EU AI log can be deleted by this purge before ~2031-06 (and the `ai_api_logs` table itself dates from 2026-02-21, so ~2031-02 is the earliest conceivable match at all). The mechanism is not merely assumed: `spec/models/ai_api_log_spec.rb:550-586` drives a real EU user through `EuJurisdiction.retention_stamp` and the purge deletes that row while sparing an `:unknown` row, so "dormant" here means no eligible production data, NOT an unproven or broken control. See `docs/legal/2026-08-23_article-50-production-flag-verification.md`
...
dormant, not un-inerted. The stamp begins writing on the first AI call logged for a confirmed EU user;
the purge begins deleting only five years after that earliest stamped row, i.e. not before ~2031-06
(`purge_old_eu_logs!` filters `created_at < 5.years.ago`; the `jurisdiction` column dates from
2026-06-21; the table itself dates from 2026-02-21). The mechanism is not merely assumed: `spec/models/ai_api_log_spec.rb:550-586` drives a real EU user through `EuJurisdiction.retention_stamp` and the purge deletes that row while sparing an `:unknown` row, so "dormant" here means no eligible production data, NOT an unproven or broken control.]** The
...
there as enforced **[CORRECTION 2026-09-14: "enforced" there states configured cadence, not execution; production scheduled dispatch was interrupted 2026-07-21 to 2026-09-02 (`LL-3e36a18199`; open when written, verified-closed 2026-09-18 on its liveness element, with the interruption impact assessment now carried by `LL-cbc8bc4211`).] [CORRECTION 2026-08-25: the EU 5-year leg is wired but matches zero rows, per the
...
IP redaction leg is unaffected by this correction.]** **[SCOPE CLARIFIED 2026-09-14:** "unaffected by this correction" means unaffected by the 2026-08-25 EU-population correction specifically, which concerns the absence of stamped EU rows and has no bearing on the IP redaction leg. It is **not** a statement that the IP redaction leg was operating. Production scheduled dispatch of `rake scheduler:dispatch` was interrupted from 2026-07-21 to 2026-09-02 (finding `LL-3e36a18199`; open when written, verified-closed 2026-09-18 on its liveness element, with the interruption impact assessment now carried by `LL-cbc8bc4211`), so this task was not run by the scheduler in that window; whether it ran by any other route has not been established. Captures dated 2026-09-14 record hourly execution from 2026-09-02 and one run of the daily 06:00 UTC block on each of the twelve UTC dates 2026-09-03 through 2026-09-14. Across September 3 to 14, each of the six inspected task summaries appeared once per UTC date and reported zero for its stated result. Six of the eleven daily tasks were inspected; the remainder were not queried. These observations do not verify every daily task, do not establish stored population size or contents, and do not evidence completion of downstream asynchronous work. See `docs/legal/2026-09-14_scheduler-dispatch-interruption-and-restoration.md`. Separately, scoped inspection at commit `4104b657b` finds that `ip_address` does not appear in any of the three files that reach `AiApiLog.log_ai_call` (`lib/ai_board_generator.rb:679`, `lib/ai_word_predictor.rb:408`, `lib/eval_narrator.rb:314`); other write paths have not been exhaustively inspected. At the 2026-08-17 live re-verification recorded in `docs/legal/2026-08-25_ai-data-flow-classification.md`, `ip_address` was null on all 64 `AiApiLog` rows then present; the table has not been re-queried since, so the current population is unknown.**]**;
