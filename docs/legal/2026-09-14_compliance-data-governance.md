...
> `docs/legal/2026-08-25_compliance-data-governance.md` (`DOC-f6d26afec8`), which remains frozen
> and byte-identical and was accurate for its date: Render was still a live write-frozen rollback
> fallback on 2026-08-25. That predecessor in turn superseded the CEO-attested repo-root
> `COMPLIANCE.md` (`DOC-9b299a785b`, ATTESTED 2026-08-04 by Scot Wahlquist, CEO), also frozen.
> Every attestation statement reproduced below belongs to **predecessor versions**; none of them
> attests these bytes.
>
> **Reason for supersession (2026-09-14): Render was decommissioned.** The workspace was deleted
> on 2026-09-09 (0 services, 0 databases, 0 disks). Six sites described Render as a live
> subprocessor, a retained fallback holding frozen production data, an MCP surface, or a pending
...
| 4 | Data-flow table, AI APIs row (:212 in the predecessor): status column "Not operational as of 2026-08-04" with the note "**not operational before or since**". | **Third operational-status carrier.** Corrected in the body of this successor (that row now reads "AI APIs (AWS Bedrock, serving Claude)", dated to the 2026-08-16 sweep) but undocumented here until now. ROOT CAUSE of the late catch: the sweep searched for the two HEADLINE PHRASES rather than enumerating every status-bearing field in every table, so a status column worded differently survived. The counter-measure is field enumeration; an earlier draft of this row wrongly blamed grep case-sensitivity, which would have sent the next sweep to add `-i` and miss it again. |
...
**Attested (PREDECESSOR versions only -- NOT these bytes):** 2026-07-06; re-attested 2026-07-23 by Scot Wahlquist, CEO. The 2026-07-06 attestation
...
| ~~Render (status/deploy)~~ ENDED 2026-09-09 | Was a hosting management dashboard; account deleted | None. No longer connected |
...
| n8n            | Workflow automation    | Workflows could be configured to move user data   | Self-hosted on GCP Cloud Run since the 2026-09-09 Render decommission; workflows reviewed in audit |
...
| Render fallback **(ENDED 2026-09-09)** | Decommissioned; workspace deleted | No data flows to Render going forward. Whether Render retains residual copies after account deletion is NOT confirmed with the vendor. The final pre-cutover database set was archived by LingoLinq to `gs://lingolinq-prod-render-archive` on 2026-09-08 and restore-verified | Render DPA retained post-termination. Residual vendor-side copies not confirmed |
...
|----------------------|-----------------------------------------------------|-----------------------------|------------------------------------------------|
| **Render.com (ENDED 2026-09-09)** | Former primary host, then write-frozen rollback fallback. Decommissioned; workspace deleted | **DPA signed and retained post-termination; BAA never executed and no longer required** | No longer a subprocessor and no longer in any hosting path. Row retained as history. The hosting-provider BAA question is answered by GCP, not Render. |
| **AWS S3**           | User-uploaded images, symbols, board exports        | **BAA SIGNED (2026-02-07)** | Accepted on AWS account 2390-4478-5114 via AWS Artifact. See `docs/legal/2026-08-12_aws-baa-acceptance-record.md` (approved successor; `AWS_BAA_ACCEPTED.md` is superseded and its :71-73 still asserts the corrected claim) and `docs/legal/AWS_BAA_2026-02.pdf`. HIPAA-eligible services only; bucket encryption enforced. |
| **AWS (Amazon Bedrock)** | **Receiving processor for runtime AI.** `lib/ai_client.rb` sends scrubbed prompts to `bedrock-runtime.<region>.amazonaws.com` over SigV4. Last verified operational on serving revision `lingolinq-web-00020-per` (swept 2026-08-16): 63 of 64 `AiApiLog` rows carry a `user_global_id` (57 `word_prediction`, 7 `board_generation`). No later revision has been checked. Eval narration is not operational on the classic plane. See `docs/legal/2026-08-16_subprocessor-register.md:5,96,99`. | **BAA SIGNED (2026-02-07)** | Amazon Bedrock is a HIPAA-eligible service under the AWS account BAA (`docs/legal/2026-08-12_aws-baa-acceptance-record.md` (approved successor; `AWS_BAA_ACCEPTED.md` is superseded and its :71-73 still asserts the corrected claim)). This is the **operative instrument** for runtime AI egress, not the Anthropic BAA. |
| **Google Cloud Platform** (Cloud Run / Cloud SQL / Memorystore) | Live production hosting for `app.lingolinq.com` as of the 2026-07-22 Gate 1 cutover | **BAA ACCEPTED (console, 2026-07-12)** | Google Cloud HIPAA BAA + Cloud Data Processing Addendum accepted 2026-07-12, SCCs certified 2026-07-14, on project `lingolinq-prod` (formalizes the org-wide Google BAA of 2026-06-08). Console acceptance, no countersigned PDF; recorded in `docs/legal/2026-09-14_gcp-baa-accepted.md`. PHI permitted under BAA terms, which are necessary but not sufficient: HIPAA-eligible services only, encryption in transit and at rest, access controls, minimum necessary; private VPC additionally. Infrastructure BAA only; does NOT cover the AWS Bedrock AI-egress path. No Google inference path is live (the Vertex/Gemini fallback was disabled 2026-07-09 PR #570); any future Vertex AI or Gemini inference path needs per-product HIPAA covered-service verification before child data. |
...
| **n8n (self-hosted)**| Workflow definitions; data depends on workflow config | N/A: self-hosted by LingoLinq on GCP Cloud Run (relocated from Render at the 2026-09-09 decommission) | Internal automation; no customer data by policy; audit workflows quarterly |
...
| **Render (ENDED 2026-09-09)** | None | None | None | Disconnected | DPA retained post-termination | The Render MCP is no longer connected and the account no longer exists. Row retained as history |
| **Notion**             | Low             | Project pages, task databases     | None if policy followed | Notion API | No           | NEVER store user data in Notion             |
| **n8n**                | Medium          | Workflow definitions, execution logs | Medium          | Self-hosted (GCP Cloud Run) | Covered by the GCP infrastructure agreements since the 2026-09-09 relocation off Render | Audit workflow configs quarterly; no customer data by policy |
...
**Status as of the 2026-08-16 read: the model leg of this diagram is OPERATIONAL.** **CORRECTED 2026-08-25.** This paragraph previously read "Status as of 2026-08-04: ... NOT OPERATIONAL" and continued that no `lingolinq-web` revision carried a Bedrock credential, so `AiClient.configured?` was false. That described one window only. The path was operational 2026-08-03T08:23Z to 2026-08-04T06:31Z on revision `00013-76w`, during which a single internal verification call was made and logged; credentials were withdrawn on `00014-5rw` at 2026-08-04T06:31:46Z, then **re-mounted 53 minutes later** on `00015-9l9` at 2026-08-04T07:25:08Z and continuously mounted through the 2026-08-16 read (`docs/legal/2026-08-16_subprocessor-register.md:99,268`). Revision credential mounts are **not monotonic**: reading one revision does not establish the next, which is why this status is dated to its evidence rather than to the date of writing. Scoping note: `AiApiLog` records the three logged runtime seams
...
operational-status correction in `docs/legal/2026-08-12_aws-baa-acceptance-record.md` (the approved successor; `AWS_BAA_ACCEPTED.md` is superseded and its :71-73 still asserts the corrected claim).
...
| A3 | **ALWAYS** execute BAAs with active infrastructure subprocessors before handling HIPAA data. | AWS BAA signed 2026-02-07 (see `docs/legal/`). GCP HIPAA BAA + CDPA accepted in-console 2026-07-12, SCCs 2026-07-14, on project `lingolinq-prod` (live Cloud Run / Cloud SQL / Memorystore hosting as of 2026-07-22; PHI permitted under BAA terms, necessary but not sufficient; see `docs/legal/2026-09-14_gcp-baa-accepted.md`). Render was decommissioned 2026-09-09 and is fully removed from the hosting path, so the Render-BAA precondition for hospital tenants is satisfied by removal rather than by executing a BAA. |
...
- **MCP access:** All local MCPs (GitHub, Notion, n8n, Filesystem, Sequential-thinking, Perplexity, DeepWiki, Chrome DevTools, Mermaid Chart, Jotform, Slack). The Render MCP was removed at the 2026-09-09 decommission.
...
| BAA status for AWS and GCP                      | Confirm BAAs are current and not expired. Render dropped from this review 2026-09-09 (no longer a subprocessor) | Scott W. |
