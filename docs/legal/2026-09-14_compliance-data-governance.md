...
| Render fallback **(ENDED 2026-09-09)** | Decommissioned; workspace deleted | None held by Render. The final pre-cutover database set was archived by LingoLinq to `gs://lingolinq-prod-render-archive` on 2026-09-08 and restore-verified | Render DPA retained post-termination. Residual vendor-side copies not confirmed |
...
| A3 | **ALWAYS** execute BAAs with active infrastructure subprocessors before handling HIPAA data. | AWS BAA signed 2026-02-07 (see `docs/legal/`). GCP HIPAA BAA + CDPA accepted in-console 2026-07-12, SCCs 2026-07-14, on project `lingolinq-prod` (live Cloud Run / Cloud SQL / Memorystore hosting as of 2026-07-22; PHI permitted under BAA terms, necessary but not sufficient; see `docs/legal/GCP_BAA_ACCEPTED.md`). Render was decommissioned 2026-09-09 and is fully removed from the hosting path, so the Render-BAA precondition for hospital tenants is satisfied by removal rather than by executing a BAA. |
