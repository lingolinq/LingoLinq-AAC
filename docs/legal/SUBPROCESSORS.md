...
**Last amended:** 2026-07-23 (CEO triage of the localization/speech subprocessor cluster. **Abair Irish TTS (#17) DISABLED** in code -- no DPA, so Irish TTS was turned off rather than executing one. The three findings were triaged: LL-a167848115 (TTS) **remediated** (Abair disabled + Google TTS on GA `v1` per #667 + consumer `translate_tts` fallback removed per #667); LL-c38e7da48e (translation #15) and LL-1eb9a2435b (voice audio #18) **accepted-risk** (BAA covered services, disclosed; raw-content auto-egress consent posture accepted for the pre-MVP test-only window, to revisit before real onboarding). Earlier 2026-07-23 amendment: Cloud TTS Pre-GA `v1beta1` residual remediated via PR #667. Prior 2026-07-22 amendments: (1) Gate 1 GCP cutover recorded -- GCP infrastructure host added as #19; (2) localization/speech re-scope + covered-service confirmation -- added Google Cloud Speech-to-Text (#18) and confirmed all three Google localization/speech services are BAA covered products, independently verified against Google's HIPAA covered-products list. DRAFT pending Privacy Office / CEO sign-off; see change log)
...
| 17 | Trinity College Dublin -- ADAPT Centre (abair.ie) | Irish-language (Gaeilge) text-to-speech synthesis. **DISABLED 2026-07-23** -- the Irish-TTS code paths (`lib/tts.rb`, `app/controllers/api/search_controller.rb#audio`) now short-circuit for `ga` locales and make no request to `https://abair.ie/aac_irish`. | None -- **no data flows to this endpoint** while disabled. | **No -- disabled** (no request is made) | Ireland (EU) | **DISABLED 2026-07-23 per CEO decision** rather than executing a DPA (finding LL-a167848115). Had **no DPA on file**; it is the only EU-hosted flow. Re-enabling Irish TTS requires a DPA/SCCs first. | https://abair.ie |
...
Three runtime flows send **raw, unscrubbed user content** to a third party and, unlike the AI rows in §5.1, do **not** pass through `lib/pii_scrubber.rb` (a fourth, Abair Irish TTS, was **DISABLED 2026-07-23** and no longer sends any data -- see below):
...
- **LL-a167848115 (TTS) -- remediated.** Every named sub-defect is closed: Abair disabled (this change), Google TTS repointed to the GA `v1` endpoint (#667), and the consumer `translate_tts` fallback removed (#667). The Google TTS flow that remains rides a GA covered-service endpoint under the BAA.
...
- ~~**Abair** (#17) Irish TTS~~ -- **DISABLED 2026-07-23.** Abair had no DPA on file and was the only EU-hosted flow. Rather than execute a DPA/SCCs, the CEO chose to disable Irish TTS: the `ga`-locale code paths in `lib/tts.rb` and `app/controllers/api/search_controller.rb#audio` now short-circuit and make no request to `abair.ie`. Re-enabling requires a DPA/SCCs first. Closed the Abair half of finding LL-a167848115.
