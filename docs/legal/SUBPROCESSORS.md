...
**The Pre-GA endpoint residual is now REMEDIATED (2026-07-23, PR #667).** Google's BAA **excludes Pre-GA offerings from PHI** ("Do not use Pre-GA offerings ... in connection with PHI"). Cloud Translation (v2) and Speech-to-Text (v1) always called GA endpoints. Cloud Text-to-Speech previously called the Pre-GA `v1beta1` endpoint at all six call sites; PR #667 repointed every one to the GA `v1` endpoint (`lib/tts.rb`, `app/controllers/api/search_controller.rb`), so all three Google flows now ride GA covered-service endpoints and are within BAA coverage for PHI. Remaining hardening options still open with the CEO: a per-flow COPPA-consent gate, and a feature flag to disable a flow per tenant. The cluster is tracked as findings **LL-c38e7da48e** (translation), **LL-a167848115** (TTS: Google TTS + Abair), and **LL-1eb9a2435b** (voice-audio transcription, #18); all remain **open** pending CEO triage of the residual consent/flag posture and the Abair decision. Until then these rows answer "Yes -- raw" rather than "Yes -- pseudonymized". As of 2026-07-16 `lingolinq-prod` carries only synthetic/test data (§5.7), so no real learner content transits these flows yet, but the code paths are live.

Two flows had **no contractual basis at all**. One is now removed; one remains a CEO decision:
...
- **Abair** (#17) has no DPA on file; it is the only EU-hosted flow and needs a DPA/SCCs or an Irish-TTS disable. This is a product tradeoff (disabling removes Irish-language speech) and remains an **open** CEO call. Part of finding LL-a167848115.
