...
| AI board generation + word prediction via Anthropic Claude on AWS Bedrock (Anthropic-only runtime) | `lib/ai_word_predictor.rb:168` | The prior Google Gemini fallback was DISABLED 2026-07-09 (Gemini Developer/AI-Studio endpoint data-handling terms inadequate for child data); there is no runtime Gemini path today. Runtime AI egresses to Claude on AWS Bedrock via lib/ai_client.rb, not the direct api.anthropic.com endpoint; enforced in CI by scripts/ai-endpoint-guard.sh. AWS exposes two Bedrock planes and AiClient supports both, selected by BEDROCK_PLANE: classic bedrock-runtime (the default, and the only plane this account is entitled to as of 2026-08-02) and bedrock-mantle (403 'not available for this account' on every model even with admin credentials; access request open with AWS). Classic bedrock-runtime is the verified HIPAA-eligible path under the account BAA; bedrock-mantle coverage has NOT been verified and must be confirmed before BEDROCK_PLANE=mantle is set. Only pseudonymized data is sent (see cap pii-scrubber-pseudonymization). |
