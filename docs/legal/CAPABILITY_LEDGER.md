...
| AI board generation + word prediction via Anthropic Claude on AWS Bedrock (Anthropic-only runtime); code path built, NOT operational in production | `lib/ai_word_predictor.rb:168` | Do NOT claim these AI features are live or available to users today; they are code-complete but unconfigured in production (no Bedrock credential on any lingolinq-web revision; see the 2026-08-01 correction in docs/legal/AWS_BAA_ACCEPTED.md). The prior Google Gemini fallback was DISABLED 2026-07-09 (Gemini Developer/AI-Studio endpoint data-handling terms inadequate for child data); there is no runtime Gemini path today. When configured, runtime AI egresses to Claude on AWS Bedrock (the Bedrock Mantle Messages API via lib/ai_client.rb), not the direct api.anthropic.com endpoint; enforced in CI by scripts/ai-endpoint-guard.sh. Only pseudonymized data is sent (see cap pii-scrubber-pseudonymization). |
