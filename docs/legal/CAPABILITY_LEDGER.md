...
| Signed AWS Business Associate Agreement (HIPAA-eligible AWS services in use) | `docs/legal/AWS_BAA_ACCEPTED.md:65` | Do NOT claim Bedrock inference is in use today: the BAA covers it, but no Bedrock call is made from production as of 2026-08-01 (no credential mounted; see the 2026-08-01 correction in docs/legal/AWS_BAA_ACCEPTED.md). Do NOT cite this row as evidence that the operative condition (calls signed under account 2390-4478-5114) has been verified; it has not, and the 2026-07-27 claim that it had was retracted. The AWS BAA does NOT cover a direct third-party model endpoint outside AWS (e.g. api.anthropic.com). That direct path is unused at runtime today; when used it is covered by the Anthropic HIPAA-Ready BAA (see cap anthropic-model-provider-baa). Fable/Mythos models are excluded from Bedrock HIPAA eligibility. |
...
| Jurisdiction-aware under-16 (EU GDPR Art. 8) block on the AI generation path | `lib/feature_flags.rb:234` | This gate is NOT the same as signup COPPA (settings['coppa']) or second-tier AI data-sharing VPC (settings['ai_consent'] / government-ID). It only covers EU under-16 AI enablement. |
| Zero-data-retention (ZDR) confirmed for the two in-scope Anthropic runtime models | `docs/legal/AI_DATA_SHARING_CONSENT.md:59` | ZDR is confirmed ONLY for these two specific models; it does NOT extend to any other/future Anthropic model outside the ZDR-eligible tier, and NOT to any non-Anthropic provider. ZDR is NOT a substitute for a HIPAA BAA (see cap anthropic-model-provider-baa for the executed Anthropic BAA and cap aws-s3-baa for Bedrock under the AWS BAA). Fable 5 / Mythos-class models are explicitly NOT ZDR-eligible and never receive identifiable payloads. |
| Signed BAA covering the AI model-provider (Anthropic) egress path | `docs/legal/ANTHROPIC_BAA_ACCEPTED.md:144` | Do NOT claim there is active runtime AI egress on any path today: the direct api.anthropic.com route is disabled and the Bedrock route has no credential mounted, so no runtime model egress occurs at all (see the 2026-08-01 correction in docs/legal/AWS_BAA_ACCEPTED.md). Do NOT cite this row as evidence that the Bedrock operative condition was verified; the 2026-07-27 claim that it had been was retracted. Google (Gemini) as a model provider still has no BAA; its runtime fallback remains disabled (2026-07-09). The Anthropic BAA is the still-available direct-path basis, not the active Bedrock route. |
...
| AI board generation + word prediction via Anthropic Claude on AWS Bedrock (Anthropic-only runtime); code path built, NOT operational in production | `lib/ai_word_predictor.rb:168` | Do NOT claim these AI features are live or available to users today; they are code-complete but unconfigured in production (no Bedrock credential on any lingolinq-web revision through 00011-l7f; see the 2026-08-01 correction in docs/legal/AWS_BAA_ACCEPTED.md). The prior Google Gemini fallback was DISABLED 2026-07-09 (Gemini Developer/AI-Studio endpoint data-handling terms inadequate for child data); there is no runtime Gemini path today. When configured, runtime AI egresses to Claude on AWS Bedrock via lib/ai_client.rb, not the direct api.anthropic.com endpoint; enforced in CI by scripts/ai-endpoint-guard.sh. AWS exposes two Bedrock planes and AiClient supports both, selected by BEDROCK_PLANE: classic bedrock-runtime (the default, and the only plane this account is entitled to as of 2026-08-01) and bedrock-mantle (403 'not available for this account' on every model even with admin credentials; access request open with AWS). Both planes are inside the AWS account BAA boundary, so plane selection is an entitlement/routing matter, not a data-protection one. Only pseudonymized data is sent (see cap pii-scrubber-pseudonymization). |
