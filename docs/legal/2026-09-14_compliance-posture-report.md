...
> via Path A supersession to `docs/legal/2026-08-22_compliance-posture-report.md`
> (`DOC-c5408d90b7`), which remains frozen and byte-identical and was accurate for its date:
> Render was still a live write-frozen rollback fallback on 2026-08-22.
>
...
- **Gate 1 DNS cutover completed (2026-07-22).** `app.lingolinq.com` serves from GCP Cloud Run
  with Cloud SQL and Memorystore. Render was decommissioned on 2026-09-09 (workspace deleted:
  0 services, 0 databases, 0 disks) and is no longer a rollback fallback. GCP is the only
  hosting path.
