...
> `docs/legal/2026-08-15_incident-response-breach-runbook.md` (`DOC-28f19f73e4`, ATTESTED
> 2026-08-16 by Scot Wahlquist, CEO), which remains frozen and byte-identical as the signed record
> and was accurate for its date. Every "attested" statement reproduced below, including in the
> change log, belongs to a predecessor version and is retained as history; none attests these bytes.
>
> **Reason for supersession.** The runbook told responders to treat Render as a live evidence
> source, an alert channel, a rollback target and a vendor contact. **Render was decommissioned and
> the workspace deleted on 2026-09-09**, so those steps could not be executed during a real
> incident. Changed: the §2 trigger list, the §3.1 report template, the §4.0 alert table (the Render
> alert row is removed), the §4.2 evidence list, the §4.3 containment step, the §4.5 recovery step,
> and the §7 vendor contact. Cloud SQL is now the only recovery source. The archived pre-cutover
> dump set in `gs://lingolinq-prod-render-archive` is named as evidence, never as a recovery path.
>
> **A capability was lost, not moved.** Render-side service and audit logs are no longer obtainable
> for any window, including windows before the deletion. §4.2 now says to record that explicitly
> rather than leave the step open. The attester should confirm this is acceptable.
...
- Malware, ransomware, suspicious access, or outage signal affecting Cloud Run, Cloud SQL, Memorystore, AWS/Bedrock/S3, the `gs://lingolinq-prod-render-archive` archive bucket, a workstation, or a tenant tool
...
> Trigger: [Sentry alert / GCP alert / GuardDuty / customer report / code review / third-party]
...
4. The Tech Lead preserves volatile evidence within the first hour. Snapshot these sources, with timestamps:
   ...
   - Cloud Storage audit logs for `gs://lingolinq-prod-render-archive` if the archived pre-cutover database set is implicated. Render's own service and audit logs are NO LONGER OBTAINABLE: the workspace was deleted 2026-09-09 and access ended with it. If an incident window predates 2026-09-09 and depends on Render-side logs, record that the evidence is unavailable rather than leaving the step open
...
5. If non-production services (staging, dev, n8n) are implicated, pause deployments and snapshot the affected service/database evidence. Render is no longer part of any path; if an incident concerns the archived pre-cutover data, snapshot the `gs://lingolinq-prod-render-archive` object metadata and its Cloud Storage audit logs instead.
...
3. Restore any lost data from the most recent clean backup in Google Cloud SQL (`lingolinq-prod-pg`,
   ...
   recovery. **There is no longer a Render rollback fallback**: it was decommissioned 2026-09-09.
   Cloud SQL is the only recovery source. The archived pre-cutover dump set in
   `gs://lingolinq-prod-render-archive` is an evidentiary record, not a recovery path, and is under
   a one-year retention policy that is NOT locked; see the retention schedule and the subprocessor register
   section 5.2.
...
- Render: **no longer a subprocessor** (workspace deleted 2026-09-09). Retained here only so that an incident whose window predates that date has a vendor of record; there is no active support channel and no account to raise a case against
...
|---|---|---|---|---|---|---|
| 2026-05-19 | Solo desk review by Scot Wahlquist, supported by Claude Code as drafting tool. Not a multi-person tabletop; first multi-person drill is scheduled per the Annual Audit Calendar. | §10.5 default scenario | n/a (paper exercise) | n/a (paper exercise) | n/a (paper exercise) | Mid-walkthrough additions: §4.1 step 4 fallback for creating evidence bucket inline; §4.2 step 6 fallback for router-level guard when affected route is not yet feature-flagged. |
