...
- Anthropic (designated runtime AI via AWS Bedrock/Mantle): privacy@anthropic.com and the Anthropic trust portal. The designated runtime model path is Anthropic Claude on the approved Bedrock/Mantle route (SUBPROCESSORS.md #4, HIPAA-Ready BAA). **Dormant as of 2026-08-02: there is no runtime model egress on any path.** No `lingolinq-web` revision carries a Bedrock credential, so `AiClient.configured?` is false, and the direct `api.anthropic.com` route is disabled and CI-enforced (see the 2026-08-01 correction in `docs/legal/AWS_BAA_ACCEPTED.md`, PR #725). Do not assume live AI egress when scoping an incident. Treat this dormant marking as authoritative over any older "active Anthropic/Bedrock runtime" language still present in SUBPROCESSORS.md or the capability ledger until PR #725 merges; after that merge, confirm whether credentials have been remounted before treating model-provider notification as in scope.
