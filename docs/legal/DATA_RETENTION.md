...
| AI API logs (`AiApiLog`) | EU-jurisdiction records kept 5 years, then purged; IP addresses redacted at 90 days regardless of jurisdiction; non-EU records have no automated purge yet (see note) | EU AI Act Article 50 record-keeping; GDPR data minimization; HIPAA audit | `AiApiLog.redact_old_ip_addresses!` for the 90 day IP scrub; `AiApiLog.purge_old_eu_logs!` for the 5 year EU purge (`jurisdiction = 'EU'` rows only) | Both run daily via `scheduler:dispatch` (`lib/tasks/scheduler.rake`; IP scrub PR #222). A blanket non-EU purge is intentionally deferred: these rows double as a HIPAA audit trail (6 year floor under 45 CFR 164.316(b)(2)), so a shorter default window is a separate decision, not a byproduct of the EU rule |
