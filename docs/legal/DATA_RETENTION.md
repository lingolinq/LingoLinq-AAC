...
| AI API logs (`AiApiLog`), EU-jurisdiction accounts | 5 years, **enforced today** | EU AI Act Article 50 record-keeping | `AiApiLog.purge_old_eu_logs!(years: 5)` (PR #553), daily via `scheduler:dispatch` | Currently inert in practice: jurisdiction is not yet stamped on accounts (VPC Phase 4 wires that) |
| AI API logs (`AiApiLog`), children's accounts (under 13) | 12 months, rolling, independent of account status; **decided, being rolled out** | 2026-07-09 ratified decision; COPPA 16 CFR § 312.10 | Not yet enforced in code as of this entry (2026-07-09) | Tracked in `docs/legal/AI_DATA_FLOW_CLASSIFICATION.md` section 6 |
| AI API logs (`AiApiLog`), all other accounts | 24 months; **decided, being rolled out** | 2026-07-09 ratified decision; GDPR Article 5(1)(e) storage limitation | Not yet enforced in code beyond the 90-day IP redaction below as of this entry (2026-07-09) | A blanket non-EU purge job was deferred pending a HIPAA audit-floor decision (up to 6 years may be required for hospital-linked accounts); see `docs/legal/AI_DATA_FLOW_CLASSIFICATION.md` section 6 |
