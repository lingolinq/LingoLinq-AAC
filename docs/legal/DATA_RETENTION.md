...
| AI API logs (`AiApiLog`) | Full record kept 2 years; IP addresses redacted at 90 days | GDPR data minimization; HIPAA audit | `AiApiLog.redact_old_ip_addresses!` for 90 day IP scrub; purge job at 2 years | IP redaction runs daily via `scheduler:dispatch` (`lib/tasks/scheduler.rake`, PR #222) |
