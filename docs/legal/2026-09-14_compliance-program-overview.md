...
> `docs/legal/2026-08-09_compliance-program-overview_draft.md` (`DOC-90632edc44`), which remains
> frozen and byte-identical and was accurate for its date: Render was still a live write-frozen
> rollback fallback on 2026-08-09. That predecessor in turn succeeded attested
...
>
> **Reason for supersession (2026-09-14).** Render was decommissioned and the workspace deleted on
> 2026-09-09. The subprocessor sentence listing Render as a retained rollback fallback is restated.
> Nothing else in the overview is changed. The attester must confirm the deletion date against the
> vendor record.
>
...
- We maintain a subprocessor list and sign agreements only with vendors that actually handle our
  ...
  (former primary host; the rollback fallback was decommissioned and the workspace deleted on
  2026-09-09, so Render is no longer in the hosting path and receives no LingoLinq data. Whether
  Render retains residual copies after account deletion is not confirmed with the vendor; the DPA
  is retained), Sentry (error monitoring, configured with
