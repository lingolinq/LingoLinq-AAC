...
- **Anthropic** -- `anthropic` gem (Claude on AWS Bedrock). Backs THREE AI features: board generation, word prediction (would send the composed sentence), and eval narration (would send scrubbed SETT/eval content). All opt-in + COPPA-gated + scrubbed. **All three are dormant as of 2026-07-30 and send nothing today**; see Section 3 and its status correction.
