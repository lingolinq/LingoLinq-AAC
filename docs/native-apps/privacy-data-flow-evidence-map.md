...
**Scope:** Backend (Rails 7.2) + Ember 5.12 frontend packaged as native (Cordova/Capacitor) apps. Hosting: GCP Cloud Run (migrated from Render at the 2026-07-22 cutover; the Render workspace was deleted 2026-09-09), storage AWS S3.
...
| **Audio recordings (button sounds, recorded speech)** | Client recording via `recordrtc` / Cordova media; `capabilities.record_audio` (`capabilities.js:755`); transcoded | S3 (`ButtonSound`), transcoded via AWS Elemental MediaConvert (`lib/transcoder.rb`) | AWS S3; AWS Elemental MediaConvert | Yes | App functionality | User Content > Audio Data | Audio > Voice or sound recordings | A recording can be a child's voice. Stored in S3; transcoder is AWS (HIPAA-eligible under the account BAA). |
| **Video (UserVideo)** | Upload/record; transcoded | S3, AWS Elemental MediaConvert (`lib/transcoder.rb`) | AWS S3 + MediaConvert | Yes | App functionality | User Content > Photos or Videos | Photos and videos | May depict a child. |
...
- **AWS SDKs** -- `aws-sdk-s3`, `aws-sdk-ses`, `aws-sdk-sns`, `aws-sdk-mediaconvert`, `aws-sdk-cloudfront`, `aws-sdk-rails`. Roles: S3 storage (boards, images, audio, video, log extra_data), SES email, SNS SMS/notifications, MediaConvert media, CloudFront CDN. Data egress: account PII (email render), user content (images/audio/video), usage logs.
