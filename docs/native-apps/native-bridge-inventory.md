...
|---|-----------|----------------------|---------------|--------------------|-----------|
| 1 | **SQLite offline storage** | `dbman.js:520-522`, `652-655` | `window.sqlitePlugin` (cordova-sqlite-storage) | `@capacitor-community/sqlite` (different API + sync semantics) | Core offline feature. Falls back to IndexedDB (quota-limited). Must prove data survives an in-place upgrade (offline-data-survival entry gate). |
| 2 | **File system (app + data dirs)** | `capabilities.js:1262-1267`, `1420-1430`, `1690`, `1712-1716`; `button.js:1165` | `window.resolveLocalFileSystemURL` + `cordova.file.applicationDirectory` / `dataDirectory` (cordova-plugin-file) | `@capacitor/filesystem` (more restricted; no app-bundle read on some platforms) | Local board/image/sound storage + bundled-asset reads for offline. Without it the app is web-only. Plan to preload assets into IndexedDB. |
