...
Signup never collects a name, so `User#generate_defaults` (`app/models/user.rb:1537`) seeds `settings['name'] = "No name"` and `lib/json_api/user.rb:328` serializes it as `name`. Guards of the form `name || user_name` or `(name || '').trim() || 'Friend'` therefore **never** fire — the sentinel is a non-empty string. It surfaced as "Hi No name" on the beta welcome, "NO NAME" in the account menu, and "Choose No name's home board" in the board picker. The backend value is load-bearing (`named_email`, utterance sharing) and asserted in 7 spec locations across 3 files, so do **not** change it; the Ember user model exposes `display_name`, which treats it as a sentinel and falls back to `user_name`. Use `display_name` anywhere a name is rendered; `name` remains correct for round-tripping to the server. Still raw at six supervisor/org sites that render *other* users (`share-board.hbs:91`, `organization/people.hbs:132`, `utterance.hbs:17`/`:49`, `user/log.hbs:31`, `start-codes.hbs:4`). Ref: [`2026-08-21-claim-check-quick-wins.md`](./2026-08-21-claim-check-quick-wins.md).
