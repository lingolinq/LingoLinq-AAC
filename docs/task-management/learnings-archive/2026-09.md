...
## Gotcha: WebKit's `openDatabase` is `!== undefined` but has no `bind`, and a stub `bind` hides it

With WebSQL disabled, WebKit's `window.openDatabase` is a function that masquerades as undefined: `typeof`
is `'undefined'`, `=== undefined` is false, and it has no `bind`. indexeddbshim 6.1.0 `__useShim()` checks
`openDatabase !== undefined` and then calls `.bind` unguarded, so the forced standalone call in
`capabilities.js` threw at module load and every iPad Home Screen launch hung on "something may be broken".
PR #1059 cleared the shim because its probe stubbed `openDatabase` as an ordinary function, which has `bind`.
Model this shape with `document.all`, which has the same three properties, not with `{}` or a function.
A capability guard must check the property it will dereference (`typeof x.bind === 'function'`), not only
presence. Without a Mac, an on-screen boot-error catcher (the temporary #1066 panel) captured the real
exception from the device. Cite `app/frontend/app/utils/standalone_idb_shim.js` and
`app/frontend/tests/unit/utils/standalone-idb-shim-test.js`.
