...
## [SUPERSEDED IN PART 2026-09-22: the `findings` equals `baseline` count test and a clean line-dropped pairing are necessary, not proof (a fix-one, add-one swap passes); fixing the shifted findings is preferred; and `npm run lint:js:todo` needs approval under CLAUDE.md Rule #14. See the `.eslint-todo` entry in ../LEARNINGS.md] Gotcha: the ESLint gate's `new=N` counts line shifts, not new violations — classify before believing it
