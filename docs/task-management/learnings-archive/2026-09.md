...
## Gotcha: "something may be broken" means the Sprockets bundle threw, and the standalone IndexedDB shim does not reach dbman

The production boot ladder in `app/views/boards/index.html.erb` (`checkState`) shows "something may be
broken" only when `load_state.js_retrieved` is true and `js_loaded` is still false after about 35s.
`js_loaded` is the last line of `app/assets/javascripts/application.js`, so that message means a
synchronous exception during bundle evaluation, before Ember boots; the #983 timeouts cannot help it.
Separately, `capabilities.js` forces `shimIndexedDB.__useShim()` for `navigator.standalone`, and on
WebKit `openDatabase` masquerades as undefined but is callable (r246707), so the swap happens and
`window.indexedDB` then throws `UnknownError: Web SQL is deprecated` on every call. It does not break
boot: `__useShim` never calls `openDatabase`, and dbman opens through `capabilities.idb`, captured one
line before the swap. Get the real exception from Safari Web Inspector on the device before blaming
storage. Cite `docs/task-management/2026-09-23_ipad-standalone-shim-diagnosis.md`.
