...
## Gotcha: a SAML consume that carries `user_id` is a link, and must return before the sign-in branches

`SessionController#saml_start` copies `embed`, `popout_id` and `oauth_code` into the same Redis config as `user_id`, and the sign-in branches in `saml_consume` test those keys. The link branch records the link and redirects before any device or token is created; keep it returning early if new flow keys are added.

**Citation:** `lib/saml_login_policy.rb` (`SamlLoginPolicy.record_link!`).
