...
- **Correction:** I first wrote that the app's `$.ajax` wrapper "does not settle inside a unit test". That was wrong. My probe's fake jqXHR lacked `responseText`, so `error_prep`'s `JSON.parse(xhr.responseText)` (`utils/extras.js`) threw inside jQuery's `.then` and the rejection was swallowed (the re-review showed this). The save-error test now stubs `$.realAjax` with a jQuery Deferred rejection carrying `responseText`, so the real wrapper builds the rejection. It fails against the old `err.error` read ("Could not save settings.").

## Third round (re-review of #1054)

- **Preview 500** on the editor's real payload (adversary, Medium; pre-existing on develop): `preview` validated a body equal to the default view as custom, and the parental-consent request view contains `<% if @offboarding %>`. A default-equal body is now treated as not customized, via `default_field_value?`, as `set_template!` already does. Controller spec posts the editor's exact payload.
- **Malformed placeholders** (Codex High, adversary Low): `%{ name }`, `%{name` (unterminated), `%{}` and `%%{name}` were accepted and sent literally. Any `%{` or `%<` left after the valid tokens, and `%%` before `{` or `<`, is now rejected. All 49 `en.yml` consent defaults still pass, with an edit appended.
- Accepted still, and sent literally: bare `{name}`, `{{name}}` and `%-{name}`, which i18n never treats as placeholders.
