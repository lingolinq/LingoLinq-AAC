...
## Known blocker, not caused by this change

`scripts/regenerate-register.sh` refuses to run: its citation-check gate fails on three findings
...
198 PASS / 0 FAIL. That repair is NOT included here: it touches findings outside this change's
scope and is Scot's call.
