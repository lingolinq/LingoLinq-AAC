...
ember: bash -c 'if [ -s "$HOME/.nvm/nvm.sh" ]; then source "$HOME/.nvm/nvm.sh"; elif [ -s "/usr/local/share/nvm/nvm.sh" ]; then source "/usr/local/share/nvm/nvm.sh"; fi && cd ./app/frontend/ && nvm install 18 && nvm use 18 && npx ember server --port 8184 --proxy http://127.0.0.1:5000'
