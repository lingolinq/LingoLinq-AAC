...
ember: bash -c 'if [ -s "$HOME/.nvm/nvm.sh" ]; then source "$HOME/.nvm/nvm.sh"; elif [ -s "/usr/local/share/nvm/nvm.sh" ]; then source "/usr/local/share/nvm/nvm.sh"; fi && nvm use 20 && cd ./app/frontend/ && npx ember server --port 8184 --proxy http://127.0.0.1:5000'
