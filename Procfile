...
ember: sh -c 'if [ -s "$HOME/.nvm/nvm.sh" ]; then . "$HOME/.nvm/nvm.sh"; elif [ -s "/usr/local/share/nvm/nvm.sh" ]; then . "/usr/local/share/nvm/nvm.sh"; fi && nvm install 18 && nvm use 18 && cd ./app/frontend/ && npx ember server --port 8184 --proxy http://127.0.0.1:5000'
